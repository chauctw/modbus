...
async function openCustomTagForm(ct = null) {
  ...
  $('#save').onclick = async () => {
    ...
    try {
      ...
      // Simple parsing: split by operators and parens, trim, find matches
      ...
      // Remove old sources no longer in expression (only for edit)
      if (isEdit) {
...
(async function init() {
  ...
  $('#addCustomTagBtn').addEventListener('click', () => {
    if (state.currentChannelId !== '__custom__') {
      selectCustomTagChannel();
    }
    openCustomTagForm();
  });
...
function setupTopbarAuth() {
  ...
  if (userDropdownToggle) {
    // userDropdownToggle.textContent = `${user.username} (${user.role}) ▾`;
    userDropdownToggle.textContent = `${user.username} ▾`;
  }

  if (userInfoDisplay) {
    // userInfoDisplay.textContent = `${user.username} (${user.role})`;
    userInfoDisplay.textContent = `${user.username}`;
  }

  if (userMgmtItem) {
    userMgmtItem.style.display = isAdmin() ? 'flex' : 'none';
    userMgmtItem.addEventListener('click', showUserManagement);
  }

  if (changePwItem) {
    changePwItem.addEventListener('click', openChangePasswordModal);
  }
