...
app.get('/api/stats', (req, res) => {
  ...
  const byScanRate = db.prepare(
    'SELECT scan_rate_ms, COUNT(*) count FROM tags GROUP BY scan_rate_ms ORDER BY count DESC'
  ).all();
  ...
  res.json({ totals, byDataType, byScanRate, duplicateIPs });
});
...
app.post('/api/devices', (req, res) => {
  const { channel_id, name, ip, slave_id = 1, scan_rate_ms = 1000, conn_timeout_s = 1, req_timeout_ms = 1000, byte_swap = false, word_swap = false } = req.body;
  ...
  const info = db.prepare(
    `INSERT INTO devices (channel_id,name,ip,slave_id,scan_rate_ms,conn_timeout_s,req_timeout_ms,sort_order,byte_swap,word_swap,raw_json)
     VALUES (?,?,?,?,?,?,?,?,?,?,?)`
  ).run(channel_id, name, ip, slave_id, scan_rate_ms, conn_timeout_s, req_timeout_ms, maxOrder + 1, byte_swap ? 1 : 0, word_swap ? 1 : 0, JSON.stringify(raw));
  res.json({ id: info.lastInsertRowid });
});
...
app.put('/api/devices/:id', (req, res) => {
  ...
  db.prepare(
    `UPDATE devices SET name=?, ip=?, slave_id=?, scan_rate_ms=?, conn_timeout_s=?, req_timeout_ms=?, byte_swap=?, word_swap=? WHERE id=?`
  ).run(
...
app.post('/api/devices/:id/duplicate', (req, res) => {
  ...
  const tx = db.transaction(() => {
    ...
    const info = db.prepare(
      `INSERT INTO devices (channel_id,name,ip,slave_id,scan_rate_ms,conn_timeout_s,req_timeout_ms,sort_order,byte_swap,word_swap,raw_json)
       VALUES (?,?,?,?,?,?,?,?,?,?,?)`
    ).run(dev.channel_id, name, ip, dev.slave_id, dev.scan_rate_ms, dev.conn_timeout_s, dev.req_timeout_ms, maxOrder + 1, dev.byte_swap, dev.word_swap, JSON.stringify(rawObj));
    ...
    tags.forEach((t) => {
      insTag.run(newDeviceId, t.name, t.address, t.data_type, t.rw_access, t.scan_rate_ms, t.scaling_type, t.sort_order, t.raw_json);
    });
    return newDeviceId;
  });
...
app.get('/api/devices/:deviceId/tags', (req, res) => {
  const { search = '', sort = 'sort_order', dir = 'asc', page = 1, pageSize = 200, realtime } = req.query;
  const allowedSort = ['name', 'address', 'data_type', 'rw_access', 'scan_rate_ms', 'scaling_type', 'sort_order', 'id'];
...
app.post('/api/tags', (req, res) => {
  const { device_id, name, address, data_type = 5, rw_access = 0, scan_rate_ms = 1000, scaling = null, decimals = 2, realtime_enabled = 0 } = req.body;
  ...
  const raw = {
    ...
    'servermain.TAG_SCAN_RATE_MILLISECONDS': scan_rate_ms,
  ...
  const info = db.prepare(
    `INSERT INTO tags (device_id,name,address,data_type,rw_access,scan_rate_ms,scaling_type,sort_order,decimals,realtime_enabled,raw_json)
     VALUES (?,?,?,?,?,?,?,?,?,?,?)`
  ).run(device_id, name, String(address), data_type, rw_access, scan_rate_ms, scaling ? 1 : 0, maxOrder + 1, Number.isInteger(decimals) ? decimals : 2, realtime_enabled ? 1 : 0, JSON.stringify(raw));
  res.json({ id: info.lastInsertRowid });
});
...
app.put('/api/tags/:id', (req, res) => {
  ...
  const { name, address, data_type, rw_access, scan_rate_ms, scaling, decimals, realtime_enabled } = req.body;
  ...
  if (scan_rate_ms != null) raw['servermain.TAG_SCAN_RATE_MILLISECONDS'] = scan_rate_ms;
  ...
  db.prepare(
    `UPDATE tags SET name=?, address=?, data_type=?, rw_access=?, scan_rate_ms=?, scaling_type=?, decimals=?, realtime_enabled=?, raw_json=? WHERE id=?`
  ).run(
    name ?? tag.name, address != null ? String(address) : tag.address, data_type ?? tag.data_type,
    rw_access ?? tag.rw_access, scan_rate_ms ?? tag.scan_rate_ms,
...
app.post('/api/tags/bulk-update', (req, res) => {
  ...
  const tx = db.transaction(() => {
    ids.forEach((id) => {
      ...
      if (!tag) return;
      const raw = JSON.parse(tag.raw_json);
      if (patch.scan_rate_ms != null) raw['servermain.TAG_SCAN_RATE_MILLISECONDS'] = patch.scan_rate_ms;
      if (patch.rw_access != null) raw['servermain.TAG_READ_WRITE_ACCESS'] = patch.rw_access;
      const sets = ['scan_rate_ms=?', 'rw_access=?', 'raw_json=?'];
      const vals = [patch.scan_rate_ms ?? tag.scan_rate_ms, patch.rw_access ?? tag.rw_access, JSON.stringify(raw)];
  ...
  const tx = db.transaction(() => {
    lines.forEach((line, idx) => {
      ...
      const raw = {
        ...
        'servermain.TAG_SCAN_RATE_MILLISECONDS': sr,
        'servermain.TAG_SCALING_TYPE': 0,
      };
      const info = insTag.run(device_id, name, address, dt, 0, sr, maxOrder, JSON.stringify(raw));
